...
function showProfile(e) {
	...
	if(e.dataset.userCaneditrole == 0){
		...
		showInfoUser.forEach(input =>{
			input.value = '';
			input.removeAttribute('required');
			input.classList.add('hidden');
		});
	}else{
		showInfoUserContainer.classList.remove('hidden');
		showInfoUser.forEach(input =>{
	...
	document.querySelector('#role').options.selectedIndex = e.dataset.userRoles;
	
	

	/*Button see-more-button*/
	/*var userRoles = e.dataset.userRoles;
	if (userRoles == 1) {
		const showUserInfo = document.getElementById('showInfoUser');
		if (showUserInfo.classList.contains('d-none')) {
			showUserInfo.classList.remove('d-none');
			showUserInfo.classList.disabled = false;
			showPurchaseHistory.classList.remove('d-none');
			showPurchaseHistory.classList.disabled = false;
			document.getElementById('role').classList.remove('d-none');
		}
		document.getElementById('see-more-button').setAttribute("data-user-id", e.dataset.userId);
		console.log('Test', userRoles, e.dataset.userProvince)
		Xoa du lieu bang cu
		document.getElementById('purchaseHistory').innerHTML = '';
		...
		document.getElementById('showInfoUser').remove();
		document.getElementById('showPurchaseHistory').remove();
		if (e.dataset.userId == e.dataset.userLogin) {
			document.getElementById('role').classList.add('d-none');
		}
		if (!showInfoUser.classList.contains('d-none')) {
			showInfoUser.classList.add('d-none');
			showPurchaseHistory.classList.add('d-none');
