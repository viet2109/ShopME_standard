...
public class AdminProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/pages/admin_product.jsp?page="+URLEncoder.encode("1", "UTF-8")).forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("delete_category".equals(action)) {
			...
			try {
				product = new Product(productName, categoryId, description, price, percentSale,  new SerialBlob(image.getInputStream().readAllBytes()));
